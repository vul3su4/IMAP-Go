...
// get footsites actibation link
func (n *ImapOpts) getFLXActivationLink() (string, error) {
	// connect to server
	...
	for _, box := range boxes {
		...
		for msg := range messages {
			...
			if !strings.Contains(strings.ToLower(fromaddress), "flx") {
				continue
			}
			...
			if !strings.Contains(strings.ToLower(mailsubject), "flx") {
				continue
			}
	...
	if ActivationLink == "" {
		return ActivationLink, errors.New("email not yet found")
	} else {
